...
char *convert_format_to_string(char *buff, size_t size, const char *format)
{
	...
	case S_CONV:
		...
		*buff++ = '%';
		...
		buff += format_string(buff, msg_addr.leng != 0 ? (const char *)msg_addr.text : "UNKNOWN", 0, prec, flags, end);
		break;
	    ...
	    case 'I':		/* M - Message ID */
		buff += format_string(buff, msg_id.leng != 0 ? (const char *)msg_id.text : "UNKNOWN", 0, prec, flags, end);
		break;
	    case 'Q':		/* Q - Queue ID */
		buff += format_string(buff, queue_id.leng != 0 ? (const char *)queue_id.text : "UNKNOWN", 0, prec, flags, end);
		break;
