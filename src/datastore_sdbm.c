...
NAME:
datastore_sdbm.c -- implements the datastore, using sdbm.
...
void *db_open(const char *db_file, size_t count, const char **names, dbmode_t open_mode)
{
    ...
    for (i = 0; i < handle->count; i += 1) {
      ...
      dbp = handle->dbp[i] = sdbm_open(handle->name[i], flags, 0664);

      if (dbp == NULL)
	  goto open_err;

      if (DEBUG_DATABASE(1)) {
	  dbh_print_names(dsh->dbh, "(db) sdbm_open( ");
	  fprintf(dbgout, ", %d )\n", open_mode);
      }
...
int db_delete(dsh_t *dsh, const dbv_t *token)
{
    ...
    for (i = 0; i < handle->count; i += 1) {
      dbp = handle->dbp[i];
      ret = sdbm_delete(dbp, db_key);

      if (ret != 0) {
          print_error(__FILE__, __LINE__, "(db) sdbm_delete('%s'), err: %d",
                      (char *)token->data, ret);
          exit(EX_ERROR);
      }
    }
...
int db_get_dbvalue(dsh_t *dsh, const dbv_t *token, /*@out@*/ dbv_t *val)
{
    ...
    // don't free db_data.dptr as sdbm has it's own internal buffer!
...
int db_set_dbvalue(dsh_t *dsh, const dbv_t *token, dbv_t *val)
{
    ...
    ret = sdbm_store(dbp, db_key, db_data, DBM_REPLACE);
...
int db_foreach(dsh_t *dsh, db_foreach_t hook, void *userdata)
{
    ...
    key = sdbm_firstkey(dbp);
    while (key.dptr) {
        data = sdbm_fetch(dbp, key);
        if (data.dptr) {
